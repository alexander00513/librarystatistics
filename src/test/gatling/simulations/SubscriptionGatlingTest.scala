...
/**
 * Performance test for the Subscription entity.
 */
class SubscriptionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Subscription entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all subscriptions")
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created subscription")
                .get("${new_subscription_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created subscription")
